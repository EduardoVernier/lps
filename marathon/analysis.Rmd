...
```{r}
n_groups <- 20
zebra_colormap <- rep(c("black", "purple"), 20)
...
Are faster atletes more disciplined?
```{r}
...
g_sd_df <- data.frame("gender" = character(0), "n" = numeric(0), "sd" = numeric(0), stringsAsFactors = FALSE)
for (i in 1:n_groups) {
  gender <- 'M'
  
  sd <- as.numeric(m_splits[[i]] %>%
                    select(cols) %>%
                    transform(SD=apply(., 1, sd, na.rm = TRUE)) %>%
                    summarize(sample_sd = mean(SD, na.rm = TRUE)))
  ...
  g_sd_df[nrow(g_sd_df) + 1,] = c(gender, n, sd)
  
  gender <- 'F'
  
  sd <- as.numeric(w_splits[[i]] %>%
                    select(cols) %>%
                    transform(SD=apply(., 1, sd, na.rm = TRUE)) %>%
                    summarize(sample_sd = mean(SD, na.rm = TRUE)))
...
g_sd_df$sd <- as.numeric(g_sd_df$sd)
g_sd_df
```
...
```{r}
ggplot(g_sd_df) +
  geom_point(aes(x=as.numeric(row.names(g_sd_df)), y=sd, colour=gender))
